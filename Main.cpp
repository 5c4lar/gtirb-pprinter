...
int main(int argc, char **argv)
{
    ...
    DdisasmLoaderPass::registerDatalogLoaders();
    ...
    desc.add_options()("help,h", "produce help message")("version", "display ddisasm version")(
        ...
        "Using the specified GTIRB file as a reference to import ground truth information");
    ...
    Pipeline.push<DdisasmLoaderPass>();
    if (vm.count("reference") != 0) {
        std::ifstream in(vm["reference"].as<std::string>());
        if (auto res = gtirb::IR::load(*GTIRB->Context, in)) {
            auto &referenceIr = *res.get()->modules_begin();
            Pipeline.push<CodeBlockLoaderPass>(referenceIr);
            Pipeline.push<DataBlockLoaderPass>(referenceIr);
            Pipeline.push<ProxyBlockLoaderPass>(referenceIr);
            Pipeline.push<SymbolLoaderPass>(referenceIr);
            Pipeline.push<SymbolicExpressionLoaderPass>(referenceIr);
